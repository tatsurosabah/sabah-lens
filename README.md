...
|---|---|
| **期間** | すべて / 1日 / 1週間 / 1ヶ月 |
| **重要** | 無国籍・市民権の議論に効く記事だけ（`impact` 3以上） |
| **タグ** | 無国籍・市民権・入管・摘発 など11種 |
| **未読 / 保存** | ヘッダーのアイコン |

`impact` は `fetch_news.py` の `impact_score()` が 0〜5 で付ける。外部のLLMは使わない。

収集の半分は「◯◯人のPATIを拘束／送還した」という定型報道で、これを眺めても
無国籍政策の状況は分からない。そこで **見出しが「数字＋不法滞在者＋拘束/送還」型なら
問答無用で 0**（`ROUTINE_TITLE_RE`）とし、残りを司法・立法・市民権の実体・国際機関・
統計といった観点で加点する。

> 本文だけで判定してはいけない。摘発報道は必ず根拠法「Akta Imigresen 1959/63」や
> 「法廷に送られた」に触れるので、`\bakta\b` や `mahkamah` を拾うと定型記事が
> 重要扱いになる。実際になったので、見出し優先＋語句を限定する形に直した。

同じ媒体が同じ型で書く記事（数字だけ違う）は一覧で **束ねて** 表示し、
「同様の報道をあと N 件みる」で開く。
...
2. **本文** — 「日本語訳 / 原文」を切り替えて読める。`body`（原文）と `body_ja`（訳）を
   同じ段落構成で持つ。`translate_long()` が段落の切れ目を保ったまま1500字ずつ訳す
3. 媒体ページへのリンクは末尾に補助的に置く

> **記事全文とその和訳が news.json に入り、公開URLで誰でも読める状態になる。**
> GitHub Pages は private リポジトリにしてもサイト自体は公開のまま（アクセス制限付き
> Pages は Enterprise 限定）なので、これは避けられない。ユーザー了承のうえでこの形にしている。

翻訳エンドポイントは非公式なので連続して叩くと **429** を返す。`translate()` は
429 のときだけ 6秒 → 20秒 → 45秒 と間隔を空けて粘り、それでも駄目なら諦めて
次回に持ち越す（中途半端な訳文は残さない）。

## 資料タブ

ニュースを追うだけでは問題の構造が見えないので、背景をまとめた画面を持つ。

| | 中身 | 生成元 |
|---|---|---|
| **経緯** | 1878年のスールー王国の協定から現在までの年表と「なぜ解決が難しいのか」 | `timeline.json`（手で書く） |
| **動画** | YouTube から自動収集。日本 → アメリカ → 台湾 → サバ → マレーシア → 国際 の順 | `gen_videos.py` → `videos.json` |
| **団体** | 現地NGO・行政・国際機関の一覧。**公開情報のみ** | `orgs.json`（手で書く） |

動画の収集は **APIキー不要**。YouTube 検索結果ページに埋まっている `ytInitialData`
（サーバー側で描画済みのJSON）から拾う。ChineseVocab の `gen_suggest.py` はチャンネルRSSを
使っているが、この題材は特定チャンネルに固まっていないので検索でないと拾えない。

- 媒体はチャンネル名で分類し、当たらなければ**見出しの文字種**で判定する
  （かながあれば日本、繁体字なら台湾）。ローマ字名の日本人発信者を拾うため
- `ann` のような短い綴りを境界なしで書くと "Channel" に部分一致して誤分類する
- 関連度は「無国籍そのものを指す語」に限る。`国籍` や `citizenship` だけだと
  "多国籍スタッフ" や "北朝鮮国籍" まで拾ってしまう

> **個人の連絡先はこのリポジトリに置かない。** 公開URLで誰でも読めるため、
> サバ州で無国籍問題に関わる人の名簿になってしまう。面談記録や連絡先は端末内に持つ。
